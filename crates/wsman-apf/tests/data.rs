...
use wsman_apf::message::{
    APF_CHANNEL_DATA, APF_CHANNEL_WINDOW_ADJUST, read_be32, write_be32,
};
...
fn open_session(script_tail: Vec<Event>) -> ApfSession<FakeHeci, NoHooks> {
    use wsman_apf::message::{APF_AMT_HTTP_PORT, APF_CHANNEL_OPEN, APF_CHANNEL_OPEN_CONFIRMATION, LME_RX_WINDOW_SIZE};
    let mut open = vec![0u8; 66];
    {
        let mut i = 0;
        open[i] = APF_CHANNEL_OPEN; i += 1;
        write_be32(&mut open[i..i + 4], 15); i += 4;
        open[i..i + 15].copy_from_slice(b"forwarded-tcpip"); i += 15;
        write_be32(&mut open[i..i + 4], 1); i += 4;
        write_be32(&mut open[i..i + 4], LME_RX_WINDOW_SIZE); i += 4;
        write_be32(&mut open[i..i + 4], 0xFFFF_FFFF); i += 4;
        write_be32(&mut open[i..i + 4], 9); i += 4;
        open[i..i + 9].copy_from_slice(b"127.0.0.1"); i += 9;
        write_be32(&mut open[i..i + 4], APF_AMT_HTTP_PORT); i += 4;
        write_be32(&mut open[i..i + 4], 9); i += 4;
        open[i..i + 9].copy_from_slice(b"127.0.0.1"); i += 9;
    ...
    let mut script = vec![
        Event::ExpectSend { me: ME, host: HOST, data: open },
        Event::ReturnRecv { me: ME, host: HOST, data: confirm },
    ];
...
fn recv_bytes_accumulates_channel_data_and_replies_with_window_adjust() {
    ...
    let mut s = open_session(vec![
        Event::ReturnRecv { me: ME, host: HOST, data: bytes_channel_data(1, payload) },
        Event::ExpectSend { me: ME, host: HOST, data: bytes_window_adjust(50, payload.len() as u32) },
        Event::ReturnRecv {
            me: ME, host: HOST,
            data: {
                let mut v = vec![0u8; 5];
